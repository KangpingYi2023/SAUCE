...
class Table_bucket:
    ...
    def output(self):
        ...
        print(f"id_attributes: {self.id_attributes}")
        print(f"bin_sizes: {self.bin_sizes}")
        # print(f"oned_bin_modes: {type(self.oned_bin_modes)}")
        # print(f"twod_bin_modes: {type(self.twod_bin_modes)}")
        for key in self.oned_bin_modes:
            print(f"oned_bin_modes:")
            print(f"{key}: {self.oned_bin_modes[key].shape}")

        for key in self.twod_bin_modes:
            print(f"twod_bin_modes:")
            print(f"{key}: {self.twod_bin_modes[key].shape}")
...
class Bucket_group:
    ...
    def output(self):
        print(f"lenth of buckets: {len(self.buckets)}")
        # print(f"buckets: {self.buckets}")
        print(f"start_key: {self.start_key}")
        print(f"sample_rate: {self.sample_rate}")
        print(f"lenth of bins: {len(self.bins)}")
        # print(f"bins: {self.bins}")
        print(f"primary_keys: {self.primary_keys}")
    ...
    for key in data:
        ...
        for i, bin in enumerate(bins):
            ...
            if len(curr_data) == 0:
                ...
                # print(stats.mode(curr_data))
                bin_mode = stats.mode(curr_data).count
                # bin_mode = bin_mode[0]
                if bin_mode > 1:
