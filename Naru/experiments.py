...
C2E_CONFIGS = dict()
C2E_CONFIGS['c2e-job-light'] = dict(
    EXPERIMENT_CONFIGS['job-light'], **{
        'checkpoint_to_load': 'models/job-light-pretrained.pt',
        'eval_psamples': [8000],
        'queries_csv': './queries/c2e.csv',
        'num_eval_queries_at_end': 1000000
    }
)

for name in C2E_CONFIGS:
    C2E_CONFIGS[name].update({'save_checkpoint_at_end': False})
EXPERIMENT_CONFIGS.update(C2E_CONFIGS)
